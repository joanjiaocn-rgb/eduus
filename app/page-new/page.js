...
export default function AIEDUInterface() {
  ...
  const handleGenerate = async (t) => {
    ...
    setResult({ topic: val, content: "Expert lesson plan generated successfully." });
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
      <aside className="w-80 bg-white border-r border-slate-200 p-8 flex flex-col gap-8 shrink-0 hidden lg:flex">
        ...
        <section className="space-y-6">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Lesson Context</h3>
        ...
        <section className="flex-1">
          ...
          <div className="space-y-3">
            {[
              ...
              <button key={item.name} onClick={() => item.pro ? router.push(item.link) : handleGenerate()} className={`w-full flex items-center justify-between p-4 rounded-2xl font-bold text-sm border-2 ${item.pro ? 'border-dashed border-slate-200 text-slate-400' : 'bg-brand-50 text-brand-700 border-brand-100'}`}>
        ...
        <div className="max-w-5xl mx-auto space-y-16">
          ...
          <div className="relative max-w-3xl mx-auto">
            <input 
              ...
              className="relative w-full bg-white p-8 pl-10 pr-44 rounded-[2rem] shadow-2xl border border-slate-100 outline-none text-2xl font-semibold transition-all focus:border-brand-600" 
          ...
          {result && (
            <div className="max-w-4xl mx-auto p-10 bg-white border border-slate-200 rounded-3xl shadow-sm animate-in fade-in">
              ...
              <p className="text-slate-700 leading-relaxed whitespace-pre-wrap">{result.content}</p>
            </div>
          )}
          ...
          <div className="pt-12 border-t border-slate-200 flex flex-col items-center gap-6 opacity-60">
            ...
            <div className="flex flex-wrap justify-center gap-8 text-xs font-bold text-slate-600">
              {['CCSS', 'NGSS', 'TEKS', 'FERPA Privacy Compliant'].map(item => (
                <div key={item} className="flex items-center gap-2"><div className="w-8 h-8 bg-slate-200 rounded-lg flex items-center justify-center font-black text-[9px]">{item.substring(0,4)}</div>{item}</div>
              ))}
            </div>
          </div>
        </div>
