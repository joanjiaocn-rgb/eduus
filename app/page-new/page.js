...
export default function AIEDUInterface() {
  ...
  const simulateGenerate = async (t) => {
    const val = t || topic;
    if (!val) return alert("Please enter a topic.");
    setTopic(val);
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden text-slate-900 bg-[#f8fafc] font-['Plus_Jakarta_Sans',sans-serif]">
      {/* Navbar */}
          ...
          <section className="flex-1">
            ...
            <div className="space-y-3">
              {[
                ...
                <button key={item.name} className={`w-full flex items-center justify-between p-4 rounded-2xl font-bold text-sm border ${item.pro ? 'border-dashed border-slate-200 text-slate-400' : 'bg-brand-50 text-brand-700 border-brand-100'}`}>
          ...
          <div className="max-w-5xl mx-auto space-y-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
              {[
                ...
                <button key={i} onClick={() => simulateGenerate(card.title)} className="group bg-white border border-slate-200 p-6 rounded-3xl text-left transition-all hover:-translate-y-1 hover:shadow-xl shadow-sm">
